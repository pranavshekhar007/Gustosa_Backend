...
comboProductController.put("/update", async (req, res) => {
  try {
    ...
    const comboProductData = await ComboProduct.findOne({ _id: id }).populate("productId");
    if (!comboProductData) {
...
comboProductController.put("/update-video", upload.single("productVideo"), async (req, res) => {
  try {
    const { id } = req.body;
